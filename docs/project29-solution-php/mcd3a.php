...
// outfiles
$oufile_tsv = OUTFILE_TSV;
$oufile_json = OUTFILE_JSON;
$oufile_json_pretty = OUTFILE_JSON_PRETTY;

// Android XML resource should use underscores (_) instead of dashes (-)
$oufile_xml_single = OUTFILE_XML;
$oufile_xml = OUTFILE_XML_SINGLE;
...
foreach($cats as $catno => $category) {
  ...
  foreach($data_slice as $k => $item) {
    ...
    $output_tsv_arr[] = $item . "\t" . strtoupper($cat_tag);
...
function file_remove_if_exists($filename) {
...
function android_xml_comments($str) {
  return str_repeat(" ", XML_INDENT_ONE) . "<!-- $str -->";
}
...
function android_xml_start() {
  ...
  return join(PHP_EOL, array(
    ...
    "",
    "<!-- Updated: $now -->",
    "<!-- Generated by ETL Solution found here: -->",
    "<!-- " . GIT_REPO . " -->",
    "",
    "<resources>",
    "",
...
function android_xml_stringarray_start($str) {
  ...
  // indent 4 spaces
  return str_repeat(" ", XML_INDENT_ONE) .
...
function android_xml_stringarray_end() {
  ...
  // indent 4 spaces
  return str_repeat(" ", XML_INDENT_ONE) . "</string-array>";
}
...
function android_xml_cdata($str) {
  ...
  // indent 8 spaces
  $prefix = str_repeat(" ", XML_INDENT_TWO) . "<item><![CDATA[";
  $suffix = "]]></item>";
...
function export_xml_single($xmldata, $title = SINGLEXML_TITLE,
  $varname = SINGLEXML_VARNAME) {
...
function export_xml($xmldata, $title = CATXML_TITLE,
  $varname = CATXML_VARNAME) {
